...
const splitFoods = (recipe) => {
  let result = [];
    ...
    let amount = item.indexOf(" ") + 1;
    let name = item.slice(amount);
    let index = name.indexOf(" ") + 1;
    result.push(name.slice(index));
   })
...
const removeLastCharacters = (str, numberOfCharacters) => {
  ...
  if (numberOfCharacters>str.length) {
      ...
      return str
  }else { x= str.slice(0,-numberOfCharacters)}
return x
};
...
const removeVowels = (str) => {
  let string = str.replace( /[aeiou]/g, '' );
return string
 };
...
const extractVowels = (str) => {
  let arr1= Array.prototype.filter.call(str, char => "aeiou".includes(char)).join("") 
  let string = str.replace( /[aeiou]/g, '' );
